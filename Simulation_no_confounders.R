...
# Simulation Studies:
## Mediation model without confounding variables
## Paper section 4.1
# simulation cases: 
...
simulations <- function(kk){
  ...
  hbic= c() 
  results =lapply(lamb_grid, HBIC_calc, xx=X,yy=Y,mm=M,n_imp = 8)
  for( ii in 1: ngrid){
  ...
  hbic0= c() 
  results =lapply(lamb_grid0, HBIC_calc, xx=intcpt ,yy=Y,mm=M,n_imp = 8)
  ...
  M_A = M[,A]
  Norefit =mediationInference2(X, Y, M_A, M_tld = M[,A_tld], alpha0_hat = alpha0_hat[A], 
                     alpha0_tld = alpha0_tld[A_tld], alpha1_hat = alpha1_hat)
  Refit = mediationInference2(X, Y, M_A, M_tld=M[,A_tld])
  
  # Oracle Model
  Ocl = mediationInference2(X, Y, M[,which(coefs$alpha0!=0)])
  
  # Zhou et al. (2020)'s method
  ZWZ_start = Sys.time()
  # lam_list should change base on different settings c1 or c2
  ZWZ= hilma(Y,M,X,mediation_setting='incomplete', lam_list = c(sqrt(log(p)/n)/3, sqrt(log(p)/n)/6))
...
lamb_grid = seq(0.13,0.25,length.out = ngrid) # Under full model
lamb_grid0 = seq(0.35,0.5,length.out = ngrid) # Under reduce model H0: \alpha_1=0
...
summary_fn = "results/simulation_no_confounders.csv"
for( jj in cases){
  c1 = c_gr[jj,1]
  c2 = c_gr[jj,2]
  cat("Running case ", jj, ', c1=', c1, ', c2 =',c2, '\n')
  
  individual_fn = paste("Gene_noConfounder10908_toJASA_c1_",c1,"_c2_",c2,".csv",sep="")
...
##########################################################################
# Plot Figure 1 in the paper section 4.1
summary_fn = "results/simulation_no_confounders.csv"
...
ggsave("results/no_confounder.pdf", dpi = 600, 
...
## Table 7: Estimated biases and standard deviations (in parentheses) of different methods with
## different c1 and c2 when there is absent of confounding variables. Except for c1 and c2, the
## values in this table equal 100 times of the actual ones
...
write.table(Tab3, 'results/no_confounders_table7.txt', append = FALSE, sep = " ", dec = ".",
            row.names = FALSE, col.names = TRUE,quote =FALSE)

## Table 8: Estimated standard deviations and average estimated standard errors with their stan-
##  dard deviations (in parentheses) over 500 replications with different c1 and c2 when there is
## absent of confounding variables. Except for c1 and c2, the values in this table equal 100 times
## of the actual ones.
...
write.table(Tab4, 'results/no_confounder_table8.txt', append = FALSE, sep = " ", dec = ".",
            row.names = FALSE, col.names = TRUE,quote =FALSE)
